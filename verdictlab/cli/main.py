"""Typer CLI application — command handlers for verdictlab.
...
Defines the main Typer app and registers subcommands:
    ...
    init      — Scaffold a new VerdictLab project
"""
...
from verdictlab.cli.init import scaffold_project
from verdictlab.config import load_config
from verdictlab.models.result import RunResult, TestResult, ScoreResult, Summary, TokenCount


app = typer.Typer(
    name="verdictlab",
    help="VerdictLab — Eval-driven agent testing framework",
...
def run(
    ...
    output_dir: str = typer.Option(
        "verdictlab-output", "--output-dir", "-o",
    ...
    provider: Optional[str] = typer.Option(
        None, "--provider",
        help="LLM provider override (deepseek|openai|anthropic). Defaults to verdictlab.yaml target.",
    ),
    model: Optional[str] = typer.Option(
        None, "--model",
        help="Model override (e.g. deepseek-chat). Defaults to verdictlab.yaml target.",
    ),
) -> None:
    ...
    output directory as verdictlab-output/report-<timestamp>.json.
    ...
    process report card — so `verdictlab compare a.json b.json --trajectory`
    and the dashboard trajectory view work on the same file.

    Examples:
        verdictlab run test-suites/example/suite.yaml
        verdictlab run suite.yaml --output-dir results/ --concurrency 5
        verdictlab run suite.yaml --no-llm  # dry-run for testing
    ...
    if no_llm:
        ...
        from verdictlab.models.suite import TestSuite as SuiteModel
        from verdictlab.cli.run_real import run_suite
        ...
        # Optional config (verdictlab.yaml in cwd) supplies provider/model.
        config = None
        cfg_path = Path("verdictlab.yaml")
        if cfg_path.exists():
            try:
...
def compare(
    ...
    Examples:
        verdictlab compare baseline.json candidate.json
        verdictlab compare verdictlab-output/report-old.json verdictlab-output/report-new.json
        verdictlab compare old.json new.json --trajectory --fail-on-trajectory-regression
    """
...
def gate(
    config_path: str = typer.Option(
        "verdictlab.yaml", "--config", "-c",
        help="Path to verdictlab.yaml config file",
    ),
) -> None:
    """CI gate: check test suites for regressions against baselines.

    Loads verdictlab.yaml, runs configured suites, compares against
    stored baselines, and exits 0 (pass) or 1 (fail).

    Examples:
        verdictlab gate
        verdictlab gate --config my-verdictlab.yaml
    """
    ...
    from verdictlab.gate.checker import GateChecker
...
def init(
    ...
    """Scaffold a new VerdictLab project.

    Creates verdictlab.yaml, test-suites/example/suite.yaml, and
    a .gitignore file for the output directory.

    Examples:
        verdictlab init
        verdictlab init my-project/
        verdictlab init --force   # overwrite existing config
    """
    ...
    typer.echo(f"✓ Initialized VerdictLab project in {target_dir.resolve()}")
    typer.echo(f"  Created: verdictlab.yaml")
    typer.echo(f"  Created: test-suites/example/suite.yaml")
    typer.echo(f"  Created: .gitignore")
    typer.echo("")
    typer.echo("  Next steps:")
    typer.echo(f"    1. Edit {target_dir / 'verdictlab.yaml'} with your API keys")
    typer.echo(f"    2. Run: verdictlab run test-suites/example/suite.yaml --no-llm")
    typer.echo(f"    3. When ready, run without --no-llm for real evaluation")


# ---------------------------------------------------------------------------
# verdictlab import-trajectory
# ---------------------------------------------------------------------------
...
def import_trajectory(
    ...
    their trace; verdictlab grades it. Accepts our own JSON format,
    ...
    Examples:
        ...
        verdictlab import-trajectory run_a.json --out run_a_report.json
        verdictlab import-trajectory trajectory.json --out report.md --allowed-tools search,calculator
    """
    from verdictlab.models import TestResult, Trajectory, build_summary_from_tests
    from verdictlab.trajectory.importers import (
        load_trajectories_file,
        load_trajectory_json,
    )
    from verdictlab.trajectory.metrics import (
        compute_budget,
        compute_convergence,
        compute_efficiency,
        compute_recovery,
