"""verdictlab dashboard — Streamlit UI for exploring evaluation results.
...
from verdictlab.ui.metrics import RAGAS_METRICS, metric_names
...
st.set_page_config(page_title="verdictlab — Eval Dashboard", page_icon="⚡", layout="wide")
...
def load_sample_pair(filenames: tuple[str, str]) -> dict[str, RunResult]:
    ...
    identical to what a user sees after uploading two `verdictlab run`
...
def load_uploaded_trajectory(uploaded) -> RunResult | None:
    ...
    from verdictlab.models import TestResult, build_summary_from_tests
...
with st.sidebar:
    ...
    st.divider()
    st.caption("verdictlab — eval-driven agent testing · v0.2.0")
...
st.markdown(
    """<div class="footer">
  <span>verdictlab v0.2.0 · MIT</span>
  <span><a href="https://github.com/rayyanakmal/verdictlab">GitHub</a> · Streamlit</span>
</div>""",
    unsafe_allow_html=True,
)
