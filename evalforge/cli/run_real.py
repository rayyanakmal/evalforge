"""Real-execution runner for `evalforge run`.

Wires the Executor to a configured LLM client and captures trajectories
during the run, so a single `evalforge run` produces a RunResult with
...
def _provider_from_config(config: GateConfig, role: str) -> ProviderConfig:
    ...
    raise ValueError(
        ...
        "'judge') section to evalforge.yaml, or use --provider/--model."
    )
